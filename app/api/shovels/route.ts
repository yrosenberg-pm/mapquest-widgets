  ...
  try {
    switch (endpoint) {
      ...
      case 'contractors-search': {
        ...
        const url = new URL(`${BASE_URL}/contractors/search`);
        url.searchParams.set('geo_id', geo_id);
        url.searchParams.set('permit_from', permit_from);
        url.searchParams.set('permit_to', permit_to);
        url.searchParams.set('size', searchParams.get('size') || '50');
        url.searchParams.set('include_tallies', 'true');
        ...
        const classifications = searchParams.getAll('contractor_classification_derived');
        for (const c of classifications) {
          url.searchParams.append('contractor_classification_derived', c);
        }

        const contractorName = searchParams.get('contractor_name');
        if (contractorName) url.searchParams.set('contractor_name', contractorName);

        const minPassRate = searchParams.get('contractor_min_inspection_pr');
        if (minPassRate) url.searchParams.set('contractor_min_inspection_pr', minPassRate);
